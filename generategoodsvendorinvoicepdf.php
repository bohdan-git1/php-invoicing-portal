...
if(strcmp($rowinv->invoicefromdate,$rowinv->invoicetodate) !=0 )
$kdatetxt = "This invoice is for the period of ". date('d/m/Y',strtotime($rowinv->invoicefromdate))." to ".date('d/m/Y',strtotime($rowinv->invoicetodate));
else
$kdatetxt  =  '';
...
<p style="text-align:center;">'. $kdatetxt.'</p>
...
if( strlen($rowChild->descrfromDate)>0)
	$comdescrption = $rowChild->Description.' from '.$rowChild->descrfromDate.' to '.$rowChild->descrtoDate;
else
	$comdescrption = $rowChild->Description;
 ...
 $htmlsubtxt = $rdt.'
    <td style="text-align:center">'.$rowChild->prefix.'</td>			
 	...
 	<td style="text-align:center">'.$comdescrption.'</td>
 	<td style="text-align:center">'.$rowChild->quantity.'</td>
	<td style="text-align:center">'.$rowChild->price_per_1_min.'</td>
	<td style="text-align:center">'.$rowChild->Charged_Amount.'</td>  
...
<tr>
	<td> '. date("d/m/Y",strtotime($rowinv->paiddate)).'</td> 
	<td>'.$rowinv->paidGateway.' </td>
	<td> '.$rowinv->paidTransactionid.'</td>
	<td> '.$rowinv->paidamount.'  </td>
  </tr>
...
$sqldbnk = "SELECT * FROM bankaccountdetails where id=$rowinv->invoiceLocalBank";
	$newrecdbnk = mysqli_query($_SERVER['con'],$sqldbnk);
	$rownewbnk = mysqli_fetch_object($newrecdbnk);
...
$html = $html.'
 ...
 <tr>
 <td> '.$rowinv->paymentscanbedone.' </td>
 </tr>
...
<tr> <td> Bank Name  </td> <td style="background-color:#CCFFFF"> '.$rownewbnk->bankname.'  </td> </tr>
<tr> <td> SWIFT code (BIC code)</td> <td style="background-color:#CCFFFF"> '.$rownewbnk->swiftcode.'  </td> </tr>
<tr> <td> Mashreq Bank ID  </td> <td style="background-color:#CCFFFF"> '.$rownewbnk->MashreqBankID.'  </td> </tr>
<tr> <td> Routing Code   </td>  <td style="background-color:#CCFFFF"> '.$rownewbnk->routingcode.'  </td> </tr>
<tr> <td> Account Title </td>  <td style="background-color:#CCFFFF">  '.$rownewbnk->accounttitle.' </td></tr>
<tr> <td> Account Number </td> <td style="background-color:#CCFFFF"> '.$rownewbnk->AccountNumber.' </td> </tr>
<tr> <td> Account IBAN   </td> <td style="background-color:#CCFFFF"> '.$rownewbnk->AccountIBAN.' </td> </tr>
</table>


 
 
<p style="text-align:center;border-top:2px solid #00CCCC;"><h5>'.$fromrowoldcomp->nameofcompany.','.$fromrowoldcomp->companyaddress.','.$fromrowoldcomp->country.',Tel #: '.$fromrowoldcomp->telephone.','.$fromrowoldcomp->email.'</h5> </p>
