...
echo $sqlchild = "SELECT * from ws_goodservice_clientinvoice_child WHERE invmasterid = $invmasterId";
...
<tr> <td>Payment Gateway : 
 <select name="paidGateway"   class="form-control">
  <option value=""  >select payment gateway</option>
  <option value="Paypal"  <?php if($resultinvoiceMaster->paidGateway == 'Paypal') echo 'selected' ?>   >Paypal</option>
  <option value="Skrill" <?php if($resultinvoiceMaster->paidGateway == 'Skrill') echo 'selected' ?>  >Skrill</option>
  <option value="Westernunion" <?php if($resultinvoiceMaster->paidGateway == 'Westernunion') echo 'selected' ?> >Western union</option>
  <option value="Wiretransform" <?php if($resultinvoiceMaster->paidGateway == 'Wiretransform') echo 'selected' ?> >Wire transform </option>
  <option value="Payoneer" <?php if($resultinvoiceMaster->paidGateway == 'Payoneer') echo 'selected' ?> >Payoneer</option>
  <option value="LocalBankDeposit"  <?php if($resultinvoiceMaster->paidGateway == 'LocalBankDeposit') echo 'selected' ?> >Local Bank Deposit  </option>
 <option value="other" <?php if($resultinvoiceMaster->paidGateway == 'other') echo 'selected' ?> >Other</option>


</select> 

 </td>  </tr>
<tr> <td>payment Transaction ID: <input   class="form-control" type="text"  name="paidTransactionid" value="<?php echo  $resultinvoiceMaster->paidTransactionid;?>" />   </td>  </tr>
...
if (isset($_POST['conform'])){
	...
	$paidGateway  = $_POST['paidGateway'];
	$paidTransactionid = $_POST['paidTransactionid'];
	...
	$sqlinv = "Update ws_goodservice_clientinvoice_master set 
 ...
 $sqldelete  = "delete from ws_goodservice_clientinvoice_child where 	invmasterid = $lastInserId ";
	...
	for($k=0;$k<count($prefixData);$k++){
		...
		if(  $quantity >0){
		  	$sqlchd = "INSERT INTO   ws_goodservice_clientinvoice_child (invmasterid,customerName,company_id,`account_id`, `prefix`,  `Description`, `price_per_1_min`,  `quantity`,  `BilledDuration_min`, `Charged_Amount`,fromDate,toDate,descrfromDate,descrtoDate) 
